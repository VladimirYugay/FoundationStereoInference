...
from datasets.augmentor import (SequenceDispFlowAugmentor,
                                SequenceDispSparseFlowAugmentor)
...
class StereoSequenceDataset(data.Dataset):
    ...
    def _load_vkitti2(self, depth_png):
        depth_image = cv2.imread(
            depth_png, cv2.IMREAD_ANYCOLOR | cv2.IMREAD_ANYDEPTH)
    ...
    def parse_txt_file(self, file_path):
        ...
        if intrinsic_match:
            K = np.array([list(map(float, row.split()))
                         for row in intrinsic_match.groups()])
        else:
        ...
        for frame_match in frame_pattern.finditer(data):
            ...
            pose_matrix = np.array([list(map(float, row.split()))
                                   for row in frame_match.group(3).strip().split('\n')])

            # Decompose pose matrix into R and T
            # The upper-left 3x3 part is the rotation matrix
            R = pose_matrix[:3, :3]
            # The first three elements of the fourth column is the translation vector
            T = pose_matrix[:3, 3]
    ...
    def _get_pytorch3d_camera_from_blender(self, R, T, K, image_size, scale: float) -> PerspectiveCameras:
        ...
        assert T.shape == (
            3,), f"Expected T to be a 3-element vector, but got {T.shape}"
    ...
    def _get_output_tensor(self, sample):
        ...
        if "camera" in sample:
            ...
            if sample["camera"]["left"][0][-3:] == "npz":
                ...
                fix_baseline = np.linalg.norm(
                    camera_left_RT[:3, 3] - camera_right_RT[:3, 3])
            ...
            # Sintel
                ...
                assert check == TAG_FLOAT, ' cam_read:: Wrong tag in flow file (should be: {0}, is: {1}). Big-endian machine? '.format(
                    TAG_FLOAT, check)
                camera_left_K = np.fromfile(
                    f, dtype='float64', count=9).reshape((3, 3))
                camera_left_RT = np.fromfile(
                    f, dtype='float64', count=12).reshape((3, 4))
                # From the MPI Sintel dataset website, the baseline of the cameras = 10cm = 0.1m
                fix_baseline = 0.1
                focal_length_px = camera_left_K[0][0]
                depth2disp_scale = focal_length_px * fix_baseline
            ...
            # KITTI Depth
                ...
                if P_key in calib_data:
                    P_values = np.array([float(x)
                                        for x in calib_data[P_key].split()])
                    P_matrix = P_values.reshape(3, 4)
                else:
                    raise KeyError(
                        f"Projection matrix for camera not found in calibration data")
                ...
                if T_key1 in calib_data and T_key2 in calib_data:
                    T1 = np.array([float(x)
                                  for x in calib_data[T_key1].split()])
                    T2 = np.array([float(x)
                                  for x in calib_data[T_key2].split()])
                    baseline = np.linalg.norm(T1 - T2)
                else:
                    raise KeyError(
                        f"Translation vectors for cameras not found in calibration data")

                R_key1 = 'R_rect_02'
                R_key2 = 'R_rect_03'
                if R_key1 in calib_data and R_key2 in calib_data:
                    R1 = np.array(
                        [float(x) for x in calib_data[R_key1].split()]).reshape(3, 3)
                    R2 = np.array(
                        [float(x) for x in calib_data[R_key2].split()]).reshape(3, 3)
                else:
                    raise KeyError(
                        f"Rotation vectors for cameras not found in calibration data")
            ...
            # SouthKensington
                ...
                camera_left_K, frames = self.parse_txt_file(
                    sample["camera"]["left"][0])
                fix_baseline = 0.12
            ...
            output_tensor["depth2disp_scale"] = [depth2disp_scale]
            output_tensor["RTK"] = [
                camera_left_R, camera_left_T, camera_left_K]
            ...
            for cam in ["left", "right"]:
                ...
                if "camera" in sample:
                    ...
                    # Sintel
                        ...
                        assert check == TAG_FLOAT, ' cam_read:: Wrong tag in flow file (should be: {0}, is: {1}). Big-endian machine? '.format(
                            TAG_FLOAT, check)
                        camera_K = np.fromfile(
                            f, dtype='float64', count=9).reshape((3, 3))
                        camera_RT = np.fromfile(
                            f, dtype='float64', count=12).reshape((3, 4))
                    ...
                    # KITTI Depth
                        ...
                        if P_key in calib_data:
                            P_values = np.array(
                                [float(x) for x in calib_data[P_key].split()])
                            P_matrix = P_values.reshape(3, 4)
                        else:
                            raise KeyError(
                                f"Projection matrix for camera not found in calibration data")
                    ...
                    # VKITTI2
                        ...
                        with open(sample["camera"]["left"][0].replace("intrinsic.txt", "extrinsic.txt"), 'r') as f:
                            ...
                            # Extract rotation (3x3) and translation (3x1)
                            camera_R = np.array([
                                [float(values[2]), float(
                                    values[3]), float(values[4])],
                                [float(values[6]), float(
                                    values[7]), float(values[8])],
                                [float(values[10]), float(
                                    values[11]), float(values[12])]
                            ], dtype=np.float32)
            ...
            for cam in (0, 1):
                ...
                if "mask" in output_tensor and cam < len(output_tensor["mask"][i]):
                    mask = torch.from_numpy(
                        output_tensor["mask"][i][cam]).float()
...
class DynamicReplicaDataset(StereoSequenceDataset):
    def __init__(
            ...
            try:
                ...
                for cam in ["left", "right"]:
                    for framedata in seq_annot[seq_name][cam]:
                        ...
                        depth_path = osp.join(
                            root, split, framedata.depth.path)
                ...
                if split == "train":
                    for ref_idx in range(0, seq_len, 3):
                        step = 1 if self.sample_len == 1 else np.random.randint(
                            1, 6)
        ...
        logging.info(
            f"Added {len(self.sample_list)} from Dynamic Replica {split}")
...
class InfinigenStereoVideoDataset(StereoSequenceDataset):
    def __init__(
        ...
        for i in range(len(sequence)):
            ...
            try:
                ...
                for cam in ["left", "right"]:
                    ...
                    im_path_list = sorted(
                        glob(osp.join(sequence[i], "frames/Image/", suffix, "*.png")))
                    depth_path_list = sorted(
                        glob(osp.join(sequence[i], "frames/Depth/", suffix, "*.npy")))
                    camera_list = sorted(
                        glob(osp.join(sequence[i], "frames/camview/", suffix, "*.npz")))
                ...
                if self.split == "train":
                    ...
                    step = self.sample_len if (self.sample_len > 0) and (
                        self.sample_len < seq_len) else seq_len
        ...
        assert len(self.sample_list) > 0, "No samples found"
        print(
            f"Added {len(self.sample_list)} from Infinigen Stereo Video {split}")
        logging.info(
            f"Added {len(self.sample_list)} from Infinigen Stereo Video {split}")
    ...
    def __init__(
        ...
        for i in range(len(sequence)):
            ...
            try:
                ...
                for cam in ["left", "right"]:
                    im_path_list = sorted(
                        glob(osp.join(sequence[i], "images", cam, "*.png")))
        ...
        assert len(self.sample_list) > 0, "No samples found"
        print(
            f"Added {len(self.sample_list)} from SouthKensington Stereo Video")
        logging.info(
            f"Added {len(self.sample_list)} from SouthKensington Stereo Video")
    ...
    def __init__(
        ...
        for i in range(len(sequence_root)):
            sequence_name = os.path.basename(
                os.path.normpath(sequence_root[i]))
        ...
        for i in range(len(sequence_split)):
            sequence_name = os.path.basename(
                os.path.normpath(sequence_split[i]))
            sequence_camera = osp.join(
                image_root, sequence_name[:10], "calib_cam_to_cam.txt")
            try:
                filenames = defaultdict(lambda: defaultdict(list))
                for cam in ["left", "right"]:
                    ...
                    for j in range(len(depth_path_list)):
                        ...
                        im_name = os.path.basename(
                            os.path.normpath(depth_path))
                        im_path = osp.join(
                            image_root, sequence_name[:10], sequence_name, suffix, "data", im_name)
                        assert os.path.isfile(im_path), im_path
                        filenames["image"][cam].append(im_path)
                        filenames["camera"][cam].append(sequence_camera)
                        filenames["metadata"][cam].append(
                            [sequence_name, (370, 1224)])
                        for k in filenames.keys():
                            assert (
                                len(filenames[k][cam])
...
class VKITTI2Dataset(StereoSequenceDataset):
    def __init__(
        ...
        if self.split == "train":
            ...
            sequence_name_list = ["Scene01_15-deg-left", "Scene02_30-deg-right",
                                  "Scene06_fog", "Scene18_morning", "Scene20_rain"]
        else:
        ...
        for i in range(len(sequence_name_list)):
            ...
            try:
                for cam in [('left', 0), ('right', 1)]:
                    depth_path_list = sorted(glob(
                        osp.join(root, f"{scene}/{variation}/frames/depth/Camera_{cam[1]}/", "*.png")))
                    im_path_list = sorted(glob(
                        osp.join(root, f"{scene}/{variation}/frames/rgb/Camera_{cam[1]}/", "*.jpg")))
                    intrinsic_path = os.path.join(
                        root, f"{scene}/{variation}/intrinsic.txt")
                    assert len(depth_path_list) == len(im_path_list), [
                        len(depth_path_list), len(im_path_list)]
                    for j in range(len(depth_path_list)):
                        depth_path = depth_path_list[j]
                        assert os.path.isfile(depth_path), depth_path
                        ...
                        filenames["metadata"][cam[0]].append(
                            [sequence_name, (375, 1242)])
                        for k in filenames.keys():
                            assert (
        ...
        for ref_idx in range(0, seq_len - self.sample_len):
            ...
            for cam in ["left", "right"]:
                for idx in range(ref_idx, ref_idx + self.sample_len):
                    ...
                    sample["disparity"][cam].append(
                        disparities[cam][seq_len - idx - 1])
            self.sample_list.append(sample)
        ...
        for seq_idx in range(num_seq):
            ...
            for cam in ["left", "right"]:
                ...
                for _ in range(len(sample["image"][cam])):
                    sample["metadata"][cam].append(
                        [sequence_name, (436, 1024)])
...
def fetch_dataloader(args):
    ...
    if add_infinigensv:
        ...
        if new_dataset is None:
            ...
            new_dataset = new_dataset + infinigensv_dataset + \
                infinigensv_dataset + infinigensv_dataset
