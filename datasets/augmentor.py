...
class SequenceDispFlowAugmentor:
    ...
    def spatial_transform(self, img, disp):
        ...
        min_scale = np.maximum(
            (self.crop_size[0] + 8) /
            float(ht), (self.crop_size[1] + 8) / float(wd)
        )
        ...
        if self.yjitter:
            y0 = np.random.randint(
                2, img[0][0].shape[0] - self.crop_size[0] - 2)
            x0 = np.random.randint(
                2, img[0][0].shape[1] - self.crop_size[1] - 2)
        ...
        if np.random.rand() < self.spatial_aug_prob:
            ...
            for i in range(len(img)):
                for cam in (0, 1):
                    ...
                    if len(disp[i]) > 0:
                        disp[i][cam], valid[i][cam] = self.resize_sparse_flow_map(
                            disp[i][cam], valid[i][cam], fx=scale_x, fy=scale_y)
